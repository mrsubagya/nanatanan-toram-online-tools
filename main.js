  ...
  class Main extends Nirvana {
    ...
    start() {
      ...
      this.crystal.forEach((data) => {
        this.select("#crystaNameList")
          .item(0)
          .appendChild(
            this.node(`
          <option value="${data.get("name")}">${data
              .get("name")
              .toLowerCase()} (${data.get("type")})</option>
        `)
          );
      });
    }
    ...
    searchProcess(name) {
      ...
      this.findOne(this.crystal, "name", name)
        .then((data) => {
          ...
          if (data.get("link")) {
            this.getDowngrade(data.get("link")).then(() => {
              ...
              this.downgrade.forEach((crystal, i) => {
                ...
                stepBox += this.card(crystal, true);
                stepBox += "</li>";
                this.select("#output-down")
                  .item(0)
                  .appendChild(this.node(stepBox));
              });
            });
          } else {
            this.select("#output-down").item(0).innerHTML = "";
    ...
    card(crystal, asString = false) {
      let element = `
      <div id="crystal-${crystal.get("code")}" link="#crystal-${crystal.get(
        "link"
      )}" class="card mb-2">
        <img src="${this.crystalIcon.get(
          crystal.get("type")
        )}" alt="icon" width="20px" class="m-1 rounded">
        <div>
          <div class="d-flex align-items-center">
            <h6 class="m-0" style="font-weight: 700">${crystal.get("name")}</h6>
            <button onclick="NV.run('Main').searchAgain('${crystal.get(
              "name"
            )}')" class="btn btn-sm">🔎</button>
          </div>
          <small class="text-secondary">${crystal.get("type")}</small>
          <div class="m-0 small" style="font-weight: 500">
